...
- **The cold Vite start is blank white for a second or two.** That is
  ...
  concluding anything.
- **A reload does not forget a web font.** After a face has been fetched once,
  a reload in the same renderer can draw the canvas in that face even when the
  new document's `document.fonts` has nothing and the fetch was aborted — Chrome
  finds the typeface by family name in the renderer's cache. So "the words are
  in the system face" cannot be proven from pixels across a reload; prove the
  page's state from `document.fonts` (measured 2026-09-16,
  `e2e/subtitle-font.spec.ts`, the failed-reopen test).
...
| radiogroup "모양" / "자리" / "효과"                    | the selected subtitle's look, place and effect (ADR-0017), in the subtitle panel; radios named by the choice word (기본 / 강조 / 외침 · 아래 / 가운데 / 위 · 바로 / 서서히 / 톡 / 올라오기), `aria-checked` on the current one, arrows choose within a row; none checked when the place is one no preset names                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
...
| checked radio `::before`                               | every `role="radio"` with `aria-checked="true"` (the subtitle rows, the 영상 모양 picker) has a `::before` with `content: ""` — a 4px dot, a shape beside the colour (UX.md: never colour alone); an unchecked one has `content: none`, and the accessible name never grows a glyph: `e2e/subtitle-style.spec.ts`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |
