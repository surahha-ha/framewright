...
| button "화면 돌리기" / "화면 원래대로"    | a quarter turn (`R`, in the palette) and everything back (in the palette); `aria-disabled` when there is nothing to put back                                                                                                              |
| `clip-picture-note`                       | the one sentence under the picture sliders (as shot, what changed, and 화면 한쪽이 비어요 when a pan uncovered a side of the box)                                                                                                         |
