...
component draws where the clip *would* land (`previewGeometry`); the command is
...
the *original* inverse and adopts the newest forward, which is only sound because
...
with what export will write), and `timeline.closeGaps` removes them *on request*.
